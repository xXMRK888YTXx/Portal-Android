...
import com.xxmrk888ytxx.portal.data.WearProfilesPayload
import com.xxmrk888ytxx.portal.data.WearTransportPayload
...
class WearPortalListenerService @Inject constructor(
    ...
    override fun onDataChanged(dataEvents: DataEventBuffer) {
        dataEvents.forEach { event ->
            ...
            wearProfileRepository.updateProfiles(
                json.decodeFromString<WearProfilesPayload>(payload).profiles.map { profile ->
                    WearProfile(
                        clientId = profile.clientId,
                        name = profile.name,
                        transport = when (profile.transport) {
                            WearTransportPayload.WIFI -> WearTransport.WIFI
                            WearTransportPayload.BLUETOOTH -> WearTransport.BLUETOOTH
                        },
                        isWakeOnLanAvailable = profile.isWakeOnLanAvailable
                    )
                }
            )
        }
